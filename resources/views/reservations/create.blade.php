    ...
    <form method="POST" action="{{ route('public.reservations.store') }}"
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <select name="reservation_resource_id" class="w-full mt-1 px-3 py-2 rounded-xl bg-black/30 border border-white/10" required>
            @foreach($resources as $rs)
              <option value="{{ $rs->id }}">[{{ $rs->type }}] {{ $rs->name }} (kap {{ $rs->capacity }})</option>
            @endforeach
          </select>
        </div>
        <div>
          <label class="text-sm text-white/70">Jenis Menu</label>
          <select id="menuType" name="menu_type" class="w-full mt-1 px-3 py-2 rounded-xl bg-black/30 border border-white/10" required>
            <option value="REGULAR">REGULAR (menu biasa)</option>
            <option value="BUFFET">BUFFET (custom)</option>
          </select>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div>
          <label class="text-sm text-white/70">Pax (opsional)</label>
      ...
      <div id="regularWrap" class="space-y-3">
        ...
        <div id="itemsWrap" class="space-y-2">
          <div class="grid md:grid-cols-3 gap-2">
            <select name="items[0][product_id]" class="px-3 py-2 rounded-xl bg-black/30 border border-white/10">
      ...
      <div id="buffetWrap" class="hidden space-y-2">
        <div class="font-semibold">Buffet (sementara input total)</div>
        <div>
          <label class="text-sm text-white/70">Total Menu Buffet (Rp)</label>
          <input type="number" name="buffet_menu_total" min="0" value="{{ old('buffet_menu_total', 0) }}"
                 class="w-full mt-1 px-3 py-2 rounded-xl bg-black/30 border border-white/10">
        </div>
        <div class="text-xs text-white/50">
          Nanti kita upgrade jadi paket buffet + inventory buffet (belanja & transfer manual).
        </div>
      </div>
