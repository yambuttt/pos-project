...
@section('title', 'Edit Produk Toko')
...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-up">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-white tracking-wide">Edit Produk</h2>
            <p class="text-white/50 text-sm mt-1">Ubah informasi detail produk retail.</p>
        </div>
    </div>
    ...
    <form action="{{ route('toko.products.update', $product) }}" method="POST" class="bg-[#0a0a0a] border border-white/5 rounded-2xl p-6 md:p-8 shadow-xl space-y-6" x-data="{ hasVariants: {{ $product->has_variants ? 'true' : 'false' }}, variants: {{ $product->has_variants && $product->variants->count() > 0 ? $product->variants->map(function($v){ return ['id'=>$v->id, 'name'=>$v->name, 'sku'=>$v->sku, 'price'=>$v->price]; })->toJson() : '[{id: Date.now()}]' }} }">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" value="{{ $product->name }}" required class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 outline-none transition-all">
            </div>
            ...
            <div x-show="!hasVariants">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="price" value="{{ $product->price }}" :required="!hasVariants" class="w-full bg-black/50 border border-white/10 rounded-xl pl-12 pr-4 py-3 text-white text-sm focus:border-yellow-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="4" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:border-yellow-500 outline-none transition-all">{{ $product->description }}</textarea>
            </div>
        </div>
        ...
        <div x-show="hasVariants" x-collapse class="bg-black/30 border border-white/5 rounded-2xl p-6 space-y-4">
            ...
            <template x-for="(variant, index) in variants" :key="index">
                <div class="flex flex-col sm:flex-row gap-4 items-end bg-black/50 p-4 rounded-xl border border-white/5">
                    ...
                    <div class="flex-1 w-full">
                        ...
                        <input type="text" :name="'variants['+index+'][name]'" x-model="variant.name" placeholder="Mis: Merah - L" class="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-yellow-500 outline-none">
                    </div>
                    <div class="flex-1 w-full">
                        <label class="block text-[10px] uppercase tracking-widest text-white/40 mb-1">SKU Varian</label>
                        <input type="text" :name="'variants['+index+'][sku]'" x-model="variant.sku" placeholder="SKU-VAR-01" class="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-yellow-500 outline-none">
                    </div>
                    <div class="flex-1 w-full">
                        <label class="block text-[10px] uppercase tracking-widest text-white/40 mb-1">Harga Varian</label>
                        <input type="number" :name="'variants['+index+'][price]'" x-model="variant.price" placeholder="0" class="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-yellow-500 outline-none">
                    </div>
            ...
            <button type="button" @click="variants.push({id: null, name: '', sku: '', price: ''})" class="text-yellow-500 hover:text-yellow-400 text-sm font-bold flex items-center gap-2 mt-2">
