    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="group relative bg-[#0a0a0a] rounded-2xl p-6 border border-white/5 hover:border-yellow-500/30 transition-all duration-300 shadow-lg overflow-hidden">
            ...
            <div class="relative z-10 flex justify-between items-start">
                <div>
                    <div class="text-white/40 text-xs font-bold uppercase tracking-widest mb-1">Pendapatan Hari Ini</div>
                    <div class="text-2xl font-bold text-white mb-2">Rp {{ number_format($todayRevenue, 0, ',', '.') }}</div>
                    @if($revenueChange > 0)
                        <div class="text-xs text-green-400 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg>
                            {{ number_format($revenueChange, 1) }}% dari kemarin
                        </div>
                    @elseif($revenueChange < 0)
                        <div class="text-xs text-red-400 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path></svg>
                            {{ number_format(abs($revenueChange), 1) }}% dari kemarin
                        </div>
                    @else
                        <div class="text-xs text-white/40 font-medium flex items-center gap-1">
                            Sama dengan kemarin
                        </div>
                    @endif
                </div>
        ...
        <div class="group relative bg-[#0a0a0a] rounded-2xl p-6 border border-white/5 hover:border-yellow-500/30 transition-all duration-300 shadow-lg overflow-hidden">
            ...
            <div class="relative z-10 flex justify-between items-start">
                <div>
                    <div class="text-white/40 text-xs font-bold uppercase tracking-widest mb-1">Transaksi Hari Ini</div>
                    <div class="text-2xl font-bold text-white mb-2">{{ number_format($todayTransactions, 0, ',', '.') }}</div>
                    @if($transactionsChange > 0)
                        <div class="text-xs text-green-400 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg>
                            {{ number_format($transactionsChange, 1) }}% dari kemarin
                        </div>
                    @elseif($transactionsChange < 0)
                        <div class="text-xs text-red-400 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path></svg>
                            {{ number_format(abs($transactionsChange), 1) }}% dari kemarin
                        </div>
                    @else
                        <div class="text-xs text-white/40 font-medium flex items-center gap-1">
                            Sama dengan kemarin
        ...
        <div class="group relative bg-[#0a0a0a] rounded-2xl p-6 border border-white/5 hover:border-yellow-500/30 transition-all duration-300 shadow-lg overflow-hidden">
            ...
            <div class="relative z-10 flex justify-between items-start">
                <div>
                    <div class="text-white/40 text-xs font-bold uppercase tracking-widest mb-1">Produk Terjual Hari Ini</div>
                    <div class="text-2xl font-bold text-white mb-2">{{ number_format($todayItemsSold, 0, ',', '.') }}</div>
                    @if($itemsSoldChange > 0)
                        <div class="text-xs text-green-400 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg>
                            {{ number_format($itemsSoldChange, 1) }}% dari kemarin
                        </div>
                    @elseif($itemsSoldChange < 0)
                        <div class="text-xs text-red-400 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path></svg>
                            {{ number_format(abs($itemsSoldChange), 1) }}% dari kemarin
                        </div>
                    @else
                        <div class="text-xs text-white/40 font-medium flex items-center gap-1">
                            Sama dengan kemarin
        ...
        <div class="group relative bg-[#0a0a0a] rounded-2xl p-6 border border-white/5 hover:border-yellow-500/30 transition-all duration-300 shadow-lg overflow-hidden">
            ...
            <div class="relative z-10 flex justify-between items-start">
                <div>
                    ...
                    <div class="text-2xl font-bold {{ $totalLowStock > 0 ? 'text-yellow-400' : 'text-white' }} mb-2">{{ $totalLowStock }} Item</div>
                    <div class="text-xs text-white/50 font-medium flex items-center gap-1">
                        {{ $totalLowStock > 0 ? 'Perlu segera direstock' : 'Semua stok aman' }}
                    </div>
                </div>
                <div class="p-3 bg-yellow-500/10 rounded-xl text-yellow-500 group-hover:scale-110 transition-transform">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-white/5 text-white/80">
                    ...
                    <tr class="group hover:bg-white/5 transition-colors">
                        <td class="py-4 font-mono text-yellow-500">{{ $sale->invoice_no }}</td>
                        <td class="py-4">{{ $sale->paid_at ? $sale->paid_at->diffForHumans() : '-' }}</td>
                        ...
                        <td class="py-4 font-bold">Rp {{ number_format($sale->total_amount, 0, ',', '.') }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="py-8 text-center text-white/20 italic">Belum ada transaksi hari ini</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
