  ...
  <div class="relative min-h-screen overflow-hidden">
    ...
    <div class="relative min-h-screen">
      <div class="flex min-h-screen w-full">
        ...
        {{-- Sidebar (desktop) --}}
        <aside class="hidden lg:block m-4 w-64 rounded-[26px] border border-white/20 bg-white/10 backdrop-blur-2xl p-4">
          ...
          <nav class="mt-5 space-y-2 text-sm">
            <a href="{{ route('pegawai.dashboard') }}"
              ...
              Dashboard
              @if($isDash)<span class="absolute left-0 top-1/2 h-6 w-1 -translate-y-1/2 rounded-r bg-white"></span>@endif
            </a>

            {{-- placeholder menu absensi --}}
            <div class="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white/70">
              Absensi (coming soon)
            </div>
          </nav>
          ...
          <form method="POST" action="{{ route('logout') }}" class="mt-4">
            @csrf
            <button class="w-full rounded-xl bg-emerald-600/80 px-4 py-3 text-sm font-semibold shadow-lg shadow-emerald-900/20 hover:bg-emerald-500/80">
              Logout
            </button>
          </form>
        </aside>
        ...
        <main class="flex-1 p-4 lg:p-6">
          ...
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="hidden sm:flex items-center gap-2 rounded-2xl border border-white/20 bg-white/10 px-4 py-2 text-sm backdrop-blur-2xl">
