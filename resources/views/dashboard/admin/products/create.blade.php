...
@section('body')
  ...
  <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between mb-8">
    <div>
      <h1 class="text-3xl font-bold text-gold-gradient">Tambah Produk</h1>
      <p class="text-sm text-white/40 font-medium">Lengkapi detail produk Anda. Resep (BOM) dapat ditambahkan setelah produk dibuat.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 glass-panel p-8 rounded-[2.5rem] space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            ...
            <input name="name" value="{{ old('name') }}" placeholder="Contoh: Espresso Martini"
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              <select name="category"
                ...
                <option value="Makanan" {{ old('category') === 'Makanan' ? 'selected' : '' }} class="bg-black">Makanan</option>
                <option value="Minuman" {{ old('category') === 'Minuman' ? 'selected' : '' }} class="bg-black">Minuman</option>
                <option value="Snacks" {{ old('category') === 'Snacks' ? 'selected' : '' }} class="bg-black">Snacks</option>
              </select>
          ...
          <div class="space-y-2">
            ...
            <input name="sku" value="{{ old('sku') }}" placeholder="PRD-001"
              class="w-full rounded-2xl border border-white/5 bg-white/[0.02] px-6 py-4 text-sm text-white outline-none placeholder:text-white/20 focus:border-gold-primary/30 focus:bg-white/[0.04] transition-all">
          </div>
        ...
        <div class="space-y-2">
          ...
          <textarea name="description" rows="5" placeholder="Gambarkan cita rasa atau keunikan produk ini..."
            class="w-full rounded-2xl border border-white/5 bg-white/[0.02] px-6 py-4 text-sm text-white outline-none placeholder:text-white/20 focus:border-gold-primary/30 focus:bg-white/[0.04] transition-all">{{ old('description') }}</textarea>
        </div>
      </div>
      ...
      <div class="space-y-8">
        ...
        <div class="glass-panel p-8 rounded-[2.5rem]">
          ...
          <div id="imagePreviewContainer" class="mb-6 hidden animate-fade-in">
            <img id="imagePreview" src="#" alt="Preview" class="w-full aspect-square rounded-3xl object-cover border border-white/10 shadow-2xl">
          </div>
          ...
          <div class="relative group cursor-pointer border-2 border-dashed border-white/5 rounded-3xl p-8 text-center hover:border-gold-primary/30 hover:bg-white/[0.02] transition-all">
            ...
            <div class="flex flex-col items-center gap-2">
              ...
              <p id="uploadLabel" class="text-[10px] text-white/30 font-bold uppercase tracking-widest text-center">Upload Media</p>
            </div>
          </div>
        ...
        <div class="glass-panel p-8 rounded-[2.5rem]">
          ...
          <label class="flex items-center justify-between p-4 glass-card rounded-2xl cursor-pointer hover:bg-white/5 transition-all">
            ...
            <div class="relative inline-flex items-center cursor-pointer">
              <input name="is_active" type="checkbox" value="1" checked class="sr-only peer">
          ...
          <p class="mt-4 text-[10px] text-white/20 leading-relaxed italic">Jika dinonaktifkan, produk tidak akan muncul di layar POS kasir.</p>
        </div>
