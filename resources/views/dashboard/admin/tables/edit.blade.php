...
@section('title', 'Update Konfigurasi Meja')
...
@section('body')
  <div class="mx-auto max-w-2xl">
    ...
    <div class="flex items-center gap-4 mb-10">
      ...
      <div>
        <h1 class="text-3xl font-bold text-gold-gradient">Update Meja</h1>
        <p class="text-sm text-white/40 font-medium italic">Modifikasi identitas <span class="text-gold-primary font-bold not-italic">meja & status</span> operasional.</p>
      </div>
    </div>
    ...
    <div class="glass-panel p-8 sm:p-12 rounded-[3rem] relative overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.tables.update', $table->id) }}" class="relative z-10 space-y-10">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                ...
                <div class="space-y-3 group">
                   <label class="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1 group-focus-within:text-gold-primary transition-colors">Identitas Meja</label>
                   <div class="relative">
                ...
                <div class="flex items-center justify-between p-6 rounded-3xl bg-black/40 border border-white/5 group hover:border-emerald-500/20 transition-all">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $table->is_active) ? 'checked' : '' }} class="sr-only peer">
    ...
    <!-- METADATA -->
    <div class="mt-8 p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 flex flex-wrap items-center justify-center gap-x-8 gap-y-4">
       <div class="flex flex-col items-center">
          <span class="text-[8px] font-black text-white/20 uppercase tracking-widest">Provisioned Date</span>
          <span class="text-[10px] font-bold text-white/60 italic">{{ $table->created_at?->format('d F Y • H:i') }}</span>
       </div>
       <div class="w-px h-8 bg-white/5 hidden sm:block"></div>
       <div class="flex flex-col items-center">
          <span class="text-[8px] font-black text-white/20 uppercase tracking-widest">Table QR Token</span>
          <span class="text-[10px] font-mono text-gold-primary/40 truncate max-w-[150px]">{{ $table->qr_token }}</span>
       </div>
    </div>
  </div>
@endsection