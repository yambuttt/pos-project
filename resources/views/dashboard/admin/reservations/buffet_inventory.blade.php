...
@section('body')
  ...
  <div class="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[1fr_.95fr]">
    ...
    <div class="rounded-[26px] border border-white/20 bg-white/10 p-5 backdrop-blur-2xl sm:p-6">
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[750px] text-left text-sm">
            ...
            <tbody class="divide-y divide-white/10">
              ...
              @empty
                <tr>
                  <td colspan="4" class="px-4 py-8 text-center text-white/60">Belum ada stok buffet.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
      ...
      {{-- NEED TABLE --}}
      @if(!empty($needRows))
        <div class="mt-6 font-semibold">Kebutuhan Bahan (Estimasi dari Paket + Recipe)</div>
              ...
              <tbody class="divide-y divide-white/10">
                @foreach($needRows as $n)
                  <tr class="hover:bg-white/5">
                    ...
                    <td class="px-4 py-3">
                      {{ rtrim(rtrim(number_format($n['need'],2,'.',''), '0'), '.') }}
                    </td>
                    <td class="px-4 py-3">
                      {{ rtrim(rtrim(number_format($n['consumed'] ?? 0,2,'.',''), '0'), '.') }}
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3">
                      {{ rtrim(rtrim(number_format($n['main_stock'] ?? 0,2,'.',''), '0'), '.') }}
                    </td>
                  </tr>
                @endforeach
              </tbody>
        ...
        <div class="mt-2 text-xs text-white/60">
          “Need” dihitung dari recipe produk di paket. “Remaining” = Need - (Consumed + Stok Buffet saat ini).
          Kekurangan bisa dipenuhi lewat Belanja (Masuk Buffet) atau Transfer dari MAIN.
        </div>
      @endif
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[900px] text-left text-sm">
            ...
            <tbody class="divide-y divide-white/10">
              ...
              @empty
                <tr>
                  <td colspan="6" class="px-4 py-8 text-center text-white/60">Belum ada movement.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
