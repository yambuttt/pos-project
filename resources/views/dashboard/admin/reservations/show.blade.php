  ...
  <div class="flex items-center justify-between gap-3">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-white/70">{{ $reservation->code }} • status: <span class="font-semibold">{{ $reservation->status }}</span></p>
      </div>
    </div>
    ...
    <div class="rounded-[26px] border border-white/20 bg-white/10 p-5 backdrop-blur-2xl sm:p-6">
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div class="rounded-2xl border border-white/15 bg-white/5 p-4">
          ...
          <div class="text-sm text-white/70">[{{ $reservation->resource?->type }}] kap {{ $reservation->resource?->capacity }}</div>
        </div>
        <div class="rounded-2xl border border-white/15 bg-white/5 p-4">
      ...
      <div class="mt-5 rounded-2xl border border-white/15 bg-white/5 p-4">
        ...
        <div class="mt-3 space-y-2">
          @forelse($reservation->items as $it)
            <div class="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm">
            ...
            <div class="overflow-x-auto">
              <table class="w-full min-w-[650px] text-left text-sm">
                ...
                <tbody class="divide-y divide-white/10">
                  ...
                  @empty
                    <tr><td colspan="4" class="px-4 py-6 text-center text-white/60">Belum ada lock.</td></tr>
                  @endforelse
                </tbody>
              </table>
            </div>
    ...
    <div class="rounded-[26px] border border-white/20 bg-white/10 p-5 backdrop-blur-2xl sm:p-6">
      <div class="rounded-2xl border border-white/15 bg-white/5 p-4">
        ...
        <div class="mt-3 space-y-1 text-sm">
          <div class="flex justify-between"><span class="text-white/70">Menu</span><span>Rp {{ number_format($reservation->menu_total,0,',','.') }}</span></div>
          <div class="flex justify-between"><span class="text-white/70">Rental</span><span>Rp {{ number_format($reservation->rental_total,0,',','.') }}</span></div>
          <div class="flex justify-between font-semibold"><span>Total</span><span>Rp {{ number_format($reservation->grand_total,0,',','.') }}</span></div>
          <div class="flex justify-between"><span class="text-white/70">DP</span><span>Rp {{ number_format($reservation->dp_amount,0,',','.') }}</span></div>
          <div class="flex justify-between"><span class="text-white/70">Paid</span><span>Rp {{ number_format($reservation->paid_amount,0,',','.') }}</span></div>
          <div class="flex justify-between"><span class="text-white/70">Remaining</span><span class="text-yellow-300 font-semibold">Rp {{ number_format($remaining,0,',','.') }}</span></div>
        </div>
      </div>
        ...
        <div class="mt-3 space-y-2 text-sm">
          @forelse($reservation->payments as $p)
            <div class="rounded-xl border border-white/10 bg-white/5 px-4 py-3">
              ...
              <div class="mt-1 text-xs text-white/60">{{ $p->method }} • {{ $p->status }} • {{ optional($p->paid_at)->format('d M Y H:i') }}</div>
            </div>
          @empty
      ...
      <div class="mt-5 space-y-3">
        @if(in_array($reservation->status, ['pending_dp','draft'], true))
          <form method="POST" action="{{ route('admin.reservations.dp_paid', $reservation) }}" class="rounded-2xl border border-white/15 bg-white/5 p-4">
            @csrf
