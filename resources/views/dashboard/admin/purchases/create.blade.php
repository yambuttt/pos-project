...
@section('body')
  <div class="flex items-center justify-between gap-3">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-xl font-semibold">Buat Purchase</h1>
        <p class="text-sm text-white/70">Tambah stok bahan dari supplier atau external (pasar/random)</p>
      </div>
    </div>
  ...
  <form method="POST" action="{{ route('admin.purchases.store') }}" class="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[1.2fr_0.8fr]">
    @csrf
    ...
    <div class="rounded-[26px] border border-white/20 bg-white/10 p-5 backdrop-blur-2xl sm:p-7 space-y-4">
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label class="text-sm text-white/80">Invoice (optional)</label>
          <input name="invoice_no" value="{{ old('invoice_no') }}"
            ...
            placeholder="INV/Nota..." />
        </div>
      </div>

      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label class="text-sm text-white/80">Source Type</label>
          <select id="source_type" name="source_type"
            class="mt-2 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm outline-none focus:border-white/40">
            <option value="external" {{ old('source_type') === 'external' ? 'selected' : '' }}>External (Pasar/Random)</option>
            <option value="supplier" {{ old('source_type') === 'supplier' ? 'selected' : '' }}>Supplier</option>
          </select>
        </div>

        <div id="supplier_wrap" class="hidden">
          <label class="text-sm text-white/80">Supplier</label>
        ...
        <div id="external_wrap">
          <label class="text-sm text-white/80">Sumber External</label>
          <input name="source_name" value="{{ old('source_name') }}"
            class="mt-2 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm outline-none placeholder:text-white/40 focus:border-white/40"
            placeholder="Contoh: Pasar Minggu / Pak Budi / Tokopedia" />
        </div>
      </div>

      <div>
        <label class="text-sm text-white/80">Catatan (optional)</label>
        <textarea name="note" rows="3"
          class="mt-2 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm outline-none placeholder:text-white/40 focus:border-white/40"
          placeholder="Catatan purchase...">{{ old('note') }}</textarea>
      </div>
      ...
      <div class="mt-2">
        ...
        <div class="mt-4 flex items-center justify-between rounded-2xl border border-white/15 bg-white/10 px-4 py-3">
    ...
    <div class="space-y-5">
      ...
      <div class="rounded-[26px] border border-white/20 bg-white/10 p-5 backdrop-blur-2xl sm:p-6">
        <div class="text-sm font-semibold">Efek ke stok</div>
        <p class="mt-2 text-sm text-white/70">
          Saat disimpan:
          <br/>• stok bahan bertambah
          <br/>• movement type <span class="font-semibold text-white/90">purchase</span> dibuat otomatis
        </p>
      </div>
    </div>
  </form>
          ...
          <div data-row class="rounded-2xl border border-white/15 bg-white/10 p-4">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-[1.2fr_0.5fr_0.6fr_0.5fr] sm:items-end">
              ...
              <div class="flex items-center justify-between sm:justify-end sm:gap-3">
                <div class="text-sm font-semibold" data-subtotal>${money(0)}</div>
