...
@section('body')
  ...
  <form method="POST" action="{{ route('admin.opnames.store') }}" class="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[1.2fr_0.8fr]">
    @csrf
    ...
    <div class="rounded-[26px] border border-white/20 bg-white/10 p-5 backdrop-blur-2xl sm:p-7">
      ...
      <button class="mt-5 w-full rounded-xl bg-blue-600/85 px-5 py-3 text-sm font-semibold shadow-lg shadow-blue-900/25 hover:bg-blue-500/85">
        Simpan Draft
      </button>
    </div>
  ...
  <script>
    (function () {
      ...
      function render() {
        // Desktop
        if (rowsDesktop) rowsDesktop.innerHTML = '';
        // Mobile
      ...
      if (fillAllBtn) {
        fillAllBtn.addEventListener('click', () => {
          document.querySelectorAll('[data-physical]').forEach((inp) => {
            const wrap = inp.closest('[data-row]') || inp.parentElement;
            const system = inp.closest('tr')?.querySelector('[data-system]')?.value
                        || inp.closest('div')?.querySelector('[data-system]')?.value
                        || inp.parentElement.querySelector('[data-system]')?.value
                        || 0;
            inp.value = system;
