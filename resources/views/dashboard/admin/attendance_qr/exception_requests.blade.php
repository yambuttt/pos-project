    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <div class="mt-1 text-sm text-white/60">
          Pengajuan absensi menggunakan device pegawai lain. Wajib lokasi + QR + selfie, dan menunggu persetujuan admin.
        </div>
      </div>
            ...
            <div class="w-full shrink-0 lg:w-[320px]">
              <div class="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                ...
                @if($it->photo_path)
                  ...
                  <img class="mt-2 w-full rounded-xl border border-white/10 object-cover"
                       src="{{ \Illuminate\Support\Facades\Storage::url($it->photo_path) }}"
                       alt="Selfie" />
                @else
                ...
                @if($it->status === 'pending')
                  <div class="mt-4 flex flex-col gap-2">
                    ...
                    <form method="POST" action="{{ route('admin.attendance.exception_requests.reject', $it->id) }}" class="space-y-2">
                      @csrf
                ...
                @else
                  <div class="mt-4 text-xs text-white/60">
                    ...
                    Waktu: <span class="text-white/80">{{ $it->reviewed_at?->format('d M Y H:i') ?? '-' }}</span><br/>
