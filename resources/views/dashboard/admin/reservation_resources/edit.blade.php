...
@section('body')
  ...
  <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between mb-8">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-3xl font-bold text-gold-gradient">Edit Resource</h1>
        <p class="text-sm text-white/40 font-medium italic">Memperbarui konfigurasi <span class="text-gold-primary font-bold not-italic">#{{ $resource->name }}</span></p>
      </div>
    </div>
  ...
  <div class="glass-panel p-8 sm:p-10 rounded-[2.5rem] relative overflow-hidden">
    ...
    <form method="POST" action="{{ route('admin.reservation_resources.update', $resource) }}" class="relative z-10 space-y-10">
      @csrf
      @method('PUT')
          ...
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div class="space-y-2">
                  ...
                  <select name="type" class="w-full rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3.5 text-sm text-white outline-none focus:border-gold-primary/30 transition-all appearance-none" required>
                      <option value="TABLE" @selected($resource->type === 'TABLE')>TABLE (Meja)</option>
                      <option value="ROOM" @selected($resource->type === 'ROOM')>ROOM (Ruangan)</option>
                      <option value="HALL" @selected($resource->type === 'HALL')>HALL (Aula)</option>
                  </select>
              </div>

              <div class="space-y-2">
                  <label class="text-[9px] uppercase tracking-widest text-white/40 font-black ml-1">Kapasitas (Pax)</label>
                  <input type="number" name="capacity" value="{{ old('capacity', $resource->capacity) }}"
                      class="w-full rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3.5 text-sm text-white outline-none focus:border-gold-primary/30 transition-all"
                      required>
              </div>
          </div>
      ...
      <div class="space-y-6">
          <div class="flex items-center gap-3">
             ...
             <h3 class="text-[11px] font-black text-white uppercase tracking-[0.2em]">Skema Harga</h3>
          </div>

          <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <div class="space-y-2">
                  <label class="text-[9px] uppercase tracking-widest text-white/40 font-black ml-1">Tarif Per Jam (Rp)</label>
                  <input type="number" name="hourly_rate" value="{{ old('hourly_rate', $resource->hourly_rate) }}"
              ...
              <div class="space-y-2">
                  ...
                  <input type="number" name="flat_rate" value="{{ old('flat_rate', $resource->flat_rate) }}"
      ...
      <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div class="space-y-2">
                  ...
                  <input type="number" name="min_duration_minutes" value="{{ old('min_duration_minutes', $resource->min_duration_minutes) }}"
                      class="w-full rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3.5 text-sm text-white outline-none focus:border-gold-primary/30 transition-all">
              </div>
              <div class="space-y-2">
                  <label class="text-[9px] uppercase tracking-widest text-white/40 font-black ml-1">Buffer Waktu (Menit)</label>
                  <input type="number" name="buffer_minutes" value="{{ old('buffer_minutes', $resource->buffer_minutes) }}"
                      class="w-full rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3.5 text-sm text-white outline-none focus:border-gold-primary/30 transition-all">
              </div>
              <div class="space-y-2">
                  ...
                  <select name="is_active" class="w-full rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3.5 text-sm text-white outline-none focus:border-gold-primary/30 transition-all appearance-none">
                      <option value="1" @selected($resource->is_active)>YA (Aktif)</option>
                      <option value="0" @selected(!$resource->is_active)>TIDAK (Non-aktif)</option>
                  </select>
              </div>
          </div>
      </div>
